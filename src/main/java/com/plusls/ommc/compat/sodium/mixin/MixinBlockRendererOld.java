...
@Dependencies(dependencyList = @Dependency(modId = "sodium", version = ">=0.2", predicate = SodiumDependencyPredicate.BlockRendererOldPredicate.class))
@Pseudo
@Mixin(targets = "me.jellysquid.mods.sodium.client.render.pipeline.BlockRenderer", remap = false)
public class MixinBlockRendererOld {
    ...
    private BakedModel modifyBakedModel(BakedModel bakedModel) {
        ...
        if (WorldEaterMineHelperUtil.shouldUseCustomModel(context.state, context.pos)) {
            BakedModel customModel = WorldEaterMineHelperUtil.models.get(context.state.getBlock());
            if (customModel != null) {
